...
Caso use o Colab é possível instalar mediante os comandos abaixo:
...
**OBS:** Para utilizar no colab é necessário colocar algumas opções para que funcione
'''
def web_driver():
...
driver = web.driver()
'''
Há também um código para alterar a versão do ubuntu utilizado por ele, ao que parece o Selenium não funciona ainda com a versão mais nova do Ubuntu utilizado na VM do colab. Você pode checar tal código no início do notebook dessa pasta.
